<template>
  <main id="mapbox_container" class="mapboxBox">
    <RatioLabel data-aos="fade-up" data-aos-duration="1600" />
...
import {
  ...
  maxBound,
  durationConfig,
} from "@/assets/config/map-config.js";
const BASE_URL =
...
export default {
  components: {
    ...
    RatioLabel: defineAsyncComponent(() =>
      import("@/components/box/RatioLabel.vue")
    ),
  },
  props: {
    location: {
  ...
  data() {
    return {
      ...
      townCode: null,
      langObj: {
        COUNTYNAME: ["來源地縣市", "City/County"],
        TOWNNAME: ["行政區", "District/Town"],
        pop_work_min: ["總工作人口", "People Surveyed"],
        transport_avg: ["大眾運輸通勤人口", "Public Transit User"],
        transport_rate: ["使用大眾運輸比例", "Percentage"],
        // VILLNAME: '里',
        ...
        // pop_work_min: this.$t("mapbox.pop_work_min"),
        // transport_avg: this.$t("mapbox.transport_avg"),
        // transport_rate: this.$t("mapbox.transport_rate"),
      },
    };
  },
  mounted() {
  ...
  methods: {
    initMapBox() {
      ...
      this.MapBoxObject.on("load", () => {
        ...
        this.MapBoxObject.setLayoutProperty(
          "settlement-label",
          "visibility",
          "none"
        );
    ...
    filterLocation() {
      ...
      this.countyCode =
        this.location && this.location.id ? this.location.id.toString() : null;
    ...
    openMapboxPopup(featuresData, LngLat) {
          ...
          Object.keys(this.langObj).map((langKey) => {
            if (langKey === "transport_rate") {
              NewProp[this.langObj[langKey][lang]] = `${(
                Propertie[langKey] * 100
              ).toFixed(1)}%`;
            } else if (
              (langKey === "COUNTYNAME" || langKey === "TOWNNAME") &&
              lang === 1
            ) {
              NewProp[this.langObj[langKey][lang]] = Propertie[langKey + "EN"];
            } else {
              NewProp[this.langObj[langKey][lang]] = Propertie[langKey];
            }
          });
          return NewProp;
        }
      ...
      nextTick(() => {
        createApp(defindPopup).mount("#popup-content");
      });
    },
  },
};
